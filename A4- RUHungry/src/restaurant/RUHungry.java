...
public class RUHungry {
	...
	// WRITE YOUR CODE HERE
        int categoryCount = StdIn.readInt();
        StdIn.readLine(); // move pointer to next line
        this.categoryVar = new String[categoryCount];
        this.menuVar = new MenuNode[categoryCount];
        for (int i = 0; i < categoryCount; i++) {
            categoryVar[i] = StdIn.readLine();
            int dishCount = StdIn.readInt();
            //System.out.println(categoryVar[i] + " " + dishCount);
            StdIn.readLine();

            for (int j = 0; j < dishCount; j++) {
                String dishName = StdIn.readLine();
                int ingredientCount = StdIn.readInt();
                //System.out.println(dishName + " " + ingredientCount);
                int[] ingredientIDs = new int[ingredientCount];
                for (int z = 0; z < ingredientCount; z++) {
                    ingredientIDs[z] = StdIn.readInt();
                }

                StdIn.readLine();
                MenuNode menuNode = new MenuNode(new Dish(categoryVar[i], dishName, ingredientIDs), menuVar[i]);
                //System.out.println("adding " + menuNode.getDish().getName());
                menuVar[i] = menuNode;
            }
        }
	...
	// WRITE YOUR CODE HERE
        if (ingredientName != null) {
            ...
            node.getIngredient().updateStockLevel(stockAmountToAdd);
        ...
        for (int i = 0; i < menuVar.length; i++) {
            ...
            while (node != null) {
                ...
                for (int id : ingredientIDs) {
                    StockNode stockNode = findStockNode(id);
                    if (stockNode != null) {
                        totalCost += stockNode.getIngredient().getCost();
                    }
                }

                dish.setPriceOfDish(totalCost * 1.2);
                dish.setProfit(dish.getPriceOfDish() - totalCost);
	...
	// WRITE YOUR CODE HERE
        ...
        int[] ingredientIDs = menuNode.getDish().getStockID();
	...
	// WRITE YOUR CODE HERE
        if (checkDishAvailability(dishName, quantity)) {
            //System.out.println("adding transaction: sucessful order of " + dishName + ", quantity: " + quantity + ", profit: " + quantity * findDish(dishName).getDish().getProfit());
            TransactionData data = new TransactionData("order", dishName, quantity, quantity * findDish(dishName).getDish().getProfit(), true);
            for (int ingredientID : findDish(dishName).getDish().getStockID()) {
                String ingredientName = findStockNode(ingredientID).getIngredient().getName();
                updateStock(ingredientName, ingredientID, -1 * quantity);
            }
            ...
            MenuNode node1 = dishNode.getNextMenuNode();
            MenuNode node2 = menuVar[categoryIndex];
            //check other dish in category, need to start with next dish first and then go around
            while (node1 != null) { // first while loop
                if (node1 != dishNode) {
                    if (checkDishAvailability(node1.getDish().getName(), quantity)) {
                        TransactionData newData = new TransactionData("order", node1.getDish().getName(), quantity, quantity * node1.getDish().getProfit(), true);
                        addTransactionNode(newData);
                        for (int ingredientID : node1.getDish().getStockID()) {
                            String ingredientName = findStockNode(ingredientID).getIngredient().getName();
                            updateStock(ingredientName, ingredientID, -1 * quantity);
                        }
                        return;
                    } else {
                        TransactionData newData = new TransactionData("order", node1.getDish().getName(), quantity, 0, false);
                        addTransactionNode(newData);
                    }
                }
                node1 = node1.getNextMenuNode();
            }

            while (node2 != dishNode) {
                if (node2 != dishNode) {
                    if (checkDishAvailability(node2.getDish().getName(), quantity)) {
                        TransactionData newData = new TransactionData("order", node2.getDish().getName(), quantity, quantity * node2.getDish().getProfit(), true);
                        addTransactionNode(newData);
                        for (int ingredientID : node2.getDish().getStockID()) {
                            String ingredientName = findStockNode(ingredientID).getIngredient().getName();
                            updateStock(ingredientName, ingredientID, -1 * quantity);
                        }
                        return;
                    } else {
                        TransactionData newData = new TransactionData("order", node2.getDish().getName(), quantity, 0, false);
                        addTransactionNode(newData);
                    }
                }
                node2 = node2.getNextMenuNode();
            }
        }
	...
	// WRITE YOUR CODE HERE
        ...
        if (stockNode == null) {return;}

        if (profit() > 50 && stockNode.getIngredient().getStockLevel() >= quantity) {
            updateStock(ingredientName, stockNode.getIngredient().getID(), -1 * quantity);


            //System.out.println("adding transaction: successful donation of " + ingredientName + ", quantity: " + quantity + ", profit: 0.0");
            TransactionData data = new TransactionData("donation", ingredientName, quantity, 0, true);

            addTransactionNode(data);
        } else {
            //System.out.println("adding transaction: unsuccessful donation of " + ingredientName + ", quantity: " + quantity + ", profit: 0.0");
	...
	// WRITE YOUR CODE HERE
        StockNode stockNode = findStockNode(ingredientName);
        if (stockNode == null) {return;}
        double cost = stockNode.getIngredient().getCost() * quantity;

        if (profit() >= cost) {
            updateStock(ingredientName, stockNode.getIngredient().getID(), quantity);
            TransactionData data = new TransactionData("restock", ingredientName, quantity, -1 * cost, true);
            addTransactionNode(data);

        } else { //failed
            TransactionData data = new TransactionData("restock", ingredientName, quantity, 0, false);
            //System.out.println("restock of "+ ingredientName + " unsuccessful");
            addTransactionNode(data);
        }
    }
